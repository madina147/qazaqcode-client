...
// API URL for uploads
const API_URL = 'http://34.34.73.209/api';
...
export const ChatProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const newSocket = io(import.meta.env.VITE_API_URL || 'http://34.34.73.209', {
      auth: {
        token
      }
    }); 
  ...
  const refreshMessages = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://34.34.73.209'}/api/messages`, {
        headers: {
  ...
  const sendFile = async (file) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://34.34.73.209'}/api/messages/upload`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
